...
to_string(const std::vector<T> &in) {
  std::string result;
  for (const auto &i : in) {
    if (!result.empty()) {
      result += ", ";
...
template <typename T>
struct is_non_string_container
    : std::conjunction<is_container<T>, std::negation<is_string_type<T>>> {};

template <typename T>
constexpr bool is_non_string_container_v = is_non_string_container<T>::value;

inline std::tuple<std::string, std::function<bool(std::string)>>
Equals(const char *xIn) {
  return {xIn, [xIn](std::string In) { return xIn == In; }};
}

template <typename T>
constexpr inline std::tuple<T, std::function<bool(T)>> Equals(T &&xIn) {
  if constexpr (is_non_string_container_v<T>) {
    return {xIn, [xIn](auto In) -> bool {
              return std::equal(In.begin(), In.end(), xIn.begin(), xIn.end());
            }};
  } else {
...
That(const T1 &xResult,
     std::tuple<T2, std::function<bool(T2)>> &&xFunc) noexcept {
...
void printDuration(const std::chrono::steady_clock::time_point &start,
  ...
  if (duration.count() < 1000) {
    ...
    std::cout << "Duration: "
              << duration.count() / static_cast<double>(1000000000) << " s"
              << std::endl;
  } else if (duration.count() >= 1000000) {
    std::cout << "Duration: " << duration.count() / static_cast<double>(1000000)
              << " ms" << std::endl;
  } else if (duration.count() >= 1000) {
    std::cout << "Duration: " << duration.count() / static_cast<double>(1000)
              << " µs" << std::endl;
  }
}
